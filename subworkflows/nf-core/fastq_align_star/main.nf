...
workflow FASTQ_ALIGN_STAR {
    ...
    ch_transcripts_fasta        // channel: [ val(meta), path(fasta), path(fai) ]
    ...
    bai                 = BAM_SORT_STATS_SAMTOOLS_GENOME.out.index           // channel: [ val(meta), path(index) ]
    ...
    bai_transcript      = BAM_SORT_STATS_SAMTOOLS_TRANSCRIPTOME.out.index    // channel: [ val(meta), path(index) ]
